...
const ALL_WIDGETS: [(&str, &dyn Fn(&toml::map::Map<String, toml::Value>) -> Result<Box<dyn Widget>, failure::Error>); 6] = [
    ("box", &|_| Ok(Box::new(standard::Time::new()))),
    ("battery", &|_| Ok(Box::new(battery::Battery::new()?))),
    ("caesura", &|_| Ok(Box::new(standard::Caesura))),
    ("mpd", &|config| {
        let host = config["host"].as_str().ok_or_else(|| format_err!("MPD host should be a string"))?;
        let port = config["port"].as_integer().ok_or_else(|| format_err!("MPD port should be an integer"))?;
